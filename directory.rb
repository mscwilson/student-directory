...
def create_student(students, full_name, cohort = Time.now.strftime("%B").to_sym)
...
def input_students(students)
  ...
  while !full_input.empty? do
    ...
    if cohort != nil && months.include?(cohort.to_sym) # Checking for month full name in hash keys
      create_student(students, full_name, cohort.to_sym)
      ...
      create_student(students, full_name, unabbreviated_month[0])
    else 
      create_student(students, full_name) # If a month isn't provided, the default cohort is used
    end
  ...
  while true
    ...
    if (1..possible_files.length).include?(filenumber_choice) # Checking they chose a possible option from the list of files
      CSV.foreach(possible_files[filenumber_choice - 1]) do |line| # reading in file
        ...
        create_student(students, student_name, cohort.to_sym)
      end           
